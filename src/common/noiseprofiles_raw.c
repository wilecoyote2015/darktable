...
JsonParser *dt_noiseprofile_raw_init(const char *alternative)
{
  GError *error = NULL;
...
{
  const dt_noiseprofile_raw_t *profile_a = (dt_noiseprofile_raw_t *)a;
  const dt_noiseprofile_raw_t *profile_b = (dt_noiseprofile_raw_t *)b;
      ...
      for(int j = 0; j < n_models; j++)
      {
        ...
        if(!g_strcmp0(cimg->camera_model, json_reader_get_string_value(reader)))
        {
          ...
          for(int k = 0; k < n_profiles; k++)
          {
            ...
            dt_noiseprofile_raw_t *new_profile = (dt_noiseprofile_raw_t *)malloc(sizeof(dt_noiseprofile_raw_t));
...
void dt_noiseprofile_raw_free(gpointer data)
{
  dt_noiseprofile_raw_t *profile = (dt_noiseprofile_raw_t *)data;
  g_free(profile->name);
...
void dt_noiseprofile_raw_interpolate(
  const dt_noiseprofile_raw_t *const p1,  // the smaller iso
  const dt_noiseprofile_raw_t *const p2,  // the larger iso (can't be == iso1)
  dt_noiseprofile_raw_t *out)             // has iso initialized
{
